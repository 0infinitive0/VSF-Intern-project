-- Pre-existing bug in match_hotels_with_rooms's availability filter: it
-- counted PRICED nights with `count(*)`, but room_prices can hold more than
-- one row per night for the same room (e.g. an admin-entered price and an
-- earlier OTA crawl land on the same check_in_date with different
-- source_url values, so ux_room_prices_natural_key's unique key does not
-- collapse them). When that happens, count(*) overshoots the requested
-- night count, the equality check fails, and the hotel silently drops out of
-- every bot search that covers that night -- even though it has availability.
-- Fixing to count(DISTINCT rp.check_in_date) makes the filter count nights,
-- not rows, which is what "does every requested night have a live price"
-- actually means.
--
-- Same migration also wires the new hotels.is_active soft-delete flag into
-- both CTEs so a deactivated hotel stops surfacing in bot search results.
--
-- Signature is unchanged from the current live function (same 13 parameters
-- as 20260820_add_guest_capacity_filter_to_match_hotels_with_rooms.sql) --
-- CREATE OR REPLACE with a matching parameter list replaces the existing
-- function in place rather than creating a second overload, so every
