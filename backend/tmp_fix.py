...
with open('src/agents/supervisor.py', 'r', encoding='utf-8') as f:
    content = f.read()
...
def decide_route_by_llm(session: Any, user_input: str) -> str | None:
    ...
    try:
        ...
        if selected_route is not None and ("Recursion limit" in str(exc) or "recursion" in str(exc).lower() or type(exc).__name__ == "GraphRecursionError"):
            return selected_route
