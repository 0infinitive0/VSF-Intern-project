...
def test_tiered_attraction_search_uses_a_single_rpc_call_without_radius(monkeypatch):
    """The deployed match_attractions radius predicate always evaluates to NULL,
    discarding every row once root_latitude/root_longitude/max_radius_km are all
    supplied (plans/reports/debug-260817-0857-match-attractions-radius-filter.md).
    search_attractions_tiered now fetches one broad, threshold-only pool and never
    forwards radius to the RPC; distance-based tiering happens downstream in
    place_search.search_attraction_candidates_tiered, the only layer with
    ...
    assert len(calls) == 1
    name, params = calls[0]
    assert name == "match_attractions"
    assert params["match_threshold"] == min(
        threshold for _, threshold in supabase_search_module.ATTRACTION_SEARCH_TIERS
    )
    assert params["filter_destination_id"] == "destination-id"
    assert "root_latitude" not in params
    assert "root_longitude" not in params
    assert "max_radius_km" not in params


def test_tiered_attraction_search_widens_fetch_count_with_required_count(monkeypatch):
    calls = []
    monkeypatch.setattr(supabase_search_module, "get_embeddings", lambda: _FakeEmbeddings())
    ...
    supabase_search_module.search_attractions_tiered(
        "local activities",
        required_count=30,
        filter_destination_id="destination-id",
        root_latitude=16.0544,
    ...
    assert calls[0]["match_count"] == 450  # max(30 * 15, 150)


def test_tiered_attraction_search_still_validates_coordinates(monkeypatch):
    """root_latitude/root_longitude stay in the signature for input validation
    even though they are no longer forwarded to the RPC."""
    monkeypatch.setattr(supabase_search_module, "get_embeddings", lambda: _FakeEmbeddings())
