...
CONTRACTS: dict[str, NodeContract] = {
    ...
    "itinerary_node": NodeContract(
        reads=frozenset({"destination", "dates.start", "dates.end", "preferences.themes", "preferences.pace"}),
        writes=frozenset({"daily_preferences.*.theme", "constraints.max_items_per_day"}),
        tools=frozenset(),
    ),
