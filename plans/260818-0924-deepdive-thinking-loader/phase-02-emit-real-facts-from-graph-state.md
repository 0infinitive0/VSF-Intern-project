...
- [ ] Hình dạng `update` từng node đã được **quan sát thật**, ghi vào docstring `phase_facts.py`
- [ ] `intake_check` chở `intent` + `fields[]` đúng với câu người dùng gõ
- [ ] `hotel_search` chở `found`/`kept` khớp với số liệu tìm kiếm thật
- [ ] Field ngoài whitelist không bao giờ lọt ra — có test riêng
- [ ] Không dữ kiện → không gửi key, không gửi `null`
- [ ] Lỗi trong bộ rút không giết chat turn — có test
- [ ] `docs/chat_api_contract.md` khớp code
- [ ] Frame `phase` cũ (chỉ `key` + `at`) vẫn hợp lệ — FE chưa sửa vẫn chạy
- [ ] `test_stream_modes.py` xanh
